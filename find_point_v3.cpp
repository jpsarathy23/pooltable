...
class Point {
    ...
    bool isBetween(Point a, Point b) {
        ...
        cout << "cross " <<crossproduct<<endl;
        //if (crossproduct != 0.0) { // have to implement epsilon
        if (!equal(crossproduct,0.0)) { // have to implement epsilon
            return false;
        }
...
class Pool {
    ...
    Point getIntersectingPoint(Line l) {
        for (int i=0; i < edges; i++) {
            ...
            //if ( !equal((lines + i)->m,l.m) && !equal((lines + i)->c,l.c)) {
                Point intersection = getIntersectionPoint(*(lines + i), l);
                cout << "int x" << intersection.x << endl;
                cout << "int y" << intersection.y << endl;
                cout << (lines + i)->p1.x << "," << (lines + i)->p1.y <<endl;
                cout << (lines + i)->p2.x << "," << (lines + i)->p2.y <<endl;
                //if ((intersection.x == (lines + i)->p1.x && intersection.y == (lines + i)->p1.y) || 
                  //  (intersection.x == (lines + i)->p2.x && intersection.y == (lines + i)->p2.y) || 
                //if ((equal(intersection.x,(lines + i)->p1.x) && equal(intersection.y,(lines + i)->p1.y)) || 
                  //  (equal(intersection.x,(lines + i)->p2.x) && equal(intersection.y,(lines + i)->p2.y)) || 
                
                if (equal(intersection.x,(lines + i)->p1.x) && equal(intersection.y,(lines + i)->p1.y)) {
                    return (lines + i)->p1;   
                }
                if (equal(intersection.x,(lines + i)->p2.x) && equal(intersection.y,(lines + i)->p2.y)) {
                    return (lines + i)->p2;   
                }
                if(intersection.isBetween((lines + i)->p1, (lines + i)->p2)) {
                    return intersection;
                }
            }
        }    
    ...
    bool isCorner(Point p) {
        for (int i=0; i < edges; i++) {
            ...
            if ( equal((*(points + i)).x, p.x) && equal((*(points + i)).y,p.y)) {
                return true;
            }
        }
        return false;
